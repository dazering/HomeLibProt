...
& $toolPath/HomeLibProt.CollectionManager.exe mergebooks -i $LibraryPath -o $LibraryPath -s 10000 -p "f.usr-" -f "$datePrefix-f.usr-*.zip"

if (!$?) {
    throw "Merge monthly usr archives failed"
}

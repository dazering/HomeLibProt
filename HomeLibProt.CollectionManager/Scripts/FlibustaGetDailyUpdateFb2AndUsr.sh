...
$ToolPath/HomeLibProt.CollectionManager downloadbooks -i $LibraryPath -o $UsrUpdatesPath -s $SiteKey -r 10 -a binary

if [ $? -ne 0 ]; then
    echo "Downloading usr archives failed"
    exit 1 
fi
...
$ToolPath/HomeLibProt.CollectionManager mergebooks -i $UsrUpdatesPath -o $LibraryPath -s 10000 -p "$DatePrefix-f.usr-" -f "*.zip"

if [ $? -ne 0 ]; then
    echo "Merge daily usr archives failed"
    exit 1 
fi

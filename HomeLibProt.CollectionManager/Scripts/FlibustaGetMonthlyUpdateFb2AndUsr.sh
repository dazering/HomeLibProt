...
$ToolPath/HomeLibProt.CollectionManager mergebooks -i $LibraryPath -o $LibraryPath -s 10000 -p "f.usr-" -f "$DatePrefix-f.usr-*.zip"

if [ $? -ne 0 ]; then
    echo "Merge monthly usr archives failed"
    exit 1 
fi
